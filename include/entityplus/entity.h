...
protected:
	...
	enity_handle(detail::entity_id_t id) : id(id) {}
public:
	enity_handle() = default;
	inline bool operator<(const enity_handle &other) const { return id < other.id; }
	inline bool operator==(const enity_handle &other) const { return id == other.id; }
};

template <typename... Components, typename... Tags>
class entity<component_list<Components...>, tag_list<Tags...>> : public enity_handle {
public:
...
public:
	...
	entity(private_access, enity_handle id, entity_manager_t *entityManager) noexcept
		: id(id), entityManager(entityManager) {}
