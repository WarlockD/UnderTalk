...
namespace tailq {
	...
	struct entry_traits {
		...
		constexpr static pointer UNLINK_MAGIC() { return reinterpret_cast<pointer>(UNLINK_MAGIC_VALUE); }
	};
	...
	struct head_traits {
		...
		// works in GCC?
		//constexpr static size_t member_offset = ((member == nullptr) ? 0 : reinterpret_cast<size_t>(&(reinterpret_cast<pointer>(0)->*member)));

		constexpr static size_t member_offset() { return ((member == nullptr) ? 0 : reinterpret_cast<size_t>(&(reinterpret_cast<pointer>(0)->*member))); }
	...
	public:
		...
		void clear() { tqh_first = nullptr; tqh_last = &tqh_first; }
		...
		void push(pointer elm) {
			...
			if (curelm == nullptr)
				...
				if (ptr_before == nullptr)
					...
					traits::_insert_before(ptr_before, elm);
			}
		}
		pointer pop() {
